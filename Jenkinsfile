    ...
    withEnv(["HOME=${env.WORKSPACE}"]) {
        ...
        withCredentials([file(credentialsId: SERVER_KEY_CREDENTALS_ID, variable: 'server_key_file')]) {
            ...
            stage('Deploye Code') {
                if (isUnix()) {
                    println('Its unix')
                    rc = sh returnStatus: true, script: "${toolbelt} force:auth:jwt:grant --clientid ${SF_CONSUMER_KEY} --username ${SF_USERNAME} --jwtkeyfile ${SERVER_KEY_CREDENTALS_ID} --setdefaultdevhubusername --instanceurl ${SF_INSTANCE_URL}"
                }else{
                    println('Not unix')
